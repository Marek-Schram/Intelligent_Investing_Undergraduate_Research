"""Quarterly rebalance memo. SPEC §9, TICKET-017.
...
@dataclass
class BearCase:
    """Bear case for a position. Required per SPEC."""

    ticker: str
    bear_text: str
    falsifiers: list[str]  # Exactly three specific observables
    citations: list[str]  # Filing citations
...
def generate_memo(
    ...
    """Generate a rebalance memo. SPEC §9.

    Raises MissingBearCaseError if any buy has no bear case.
    """
    ...
    for buy in buys:
        if buy not in bear_tickers:
            raise MissingBearCaseError(
                f"Buy of {buy} has no bear case. "
                "Every new position requires a bear case with 3 falsifiers."
            )
